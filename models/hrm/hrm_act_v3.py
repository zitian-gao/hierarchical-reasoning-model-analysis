"""
HRM ACT V3: Transformer Baseline without Adaptive Computation Time (ACT)

This variant matches the ACT V2 architecture but removes the adaptive
computation mechanism, always running for the maximum number of outer steps.
...
class HierarchicalReasoningModel_ACTV3_Inner(nn.Module):
    ...
    def forward(
        self, carry: HierarchicalReasoningModel_ACTV3InnerCarry, batch: Dict[str, torch.Tensor]
...
class HierarchicalReasoningModel_ACTV3(nn.Module):
    """Wrapper that always runs for the configured number of steps (no ACT)."""
    ...
    def forward(
        ...
        # The ACT mechanism is disabled in this variant, so compute_target_q is ignored.
